...
class LocationController {
  ...
  void _updateLocationFromAccelerometer(
      AccelerometerEvent event, DateTime now) {
    if (lastAccelTime == null ||
        model.accelLatitude == null ||
        model.accelLongitude == null) {
      lastAccelTime = now;
      return;
    }
    ...
    final deltaT =
        now.difference(lastAccelTime!).inMilliseconds / 1000.0; // in seconds
    lastAccelTime = now;
    ...
    final deltaLon = (displacementX /
            (earthRadius * math.cos(model.accelLatitude! * math.pi / 180))) *
        (180 / math.pi); // degrees
